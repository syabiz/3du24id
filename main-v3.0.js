...
(() => {
  const e = doc.getElementById("slider-banner");
  if (e) {
    ...
    const c = () => {
      if (!r && win.scrollY > 0) {
        ...
        o.slideNext();
        o.autoplay.start();
        win.removeEventListener("scroll", c);
        r = true;
      }
    };
    win.addEventListener("scroll", c);
  }
})();
...
(() => {
  ...
  doc.querySelectorAll(".swiper-double").forEach(
    (t) =>
      new Swiper(t, {
        ...e,
        slidesPerView: 2,
...
function loadStyle() {
  const e = [
    ...[...doc.querySelectorAll(".style-summon")].map((n) => n.dataset.href).filter(Boolean),
  ];
...
function fLBP() {
  var e =
    html.lang == "id" ? "Belum ada postingan dengan label ini!" : "No posts with this label yet!";
  ...
  function a(e, o) {
    ...
    return e
      .replace(/\/default/g, "/hqdefault")
      .replace(/(\/|=)s72.*?-c(?:-rw)?/g, "$1w" + i + "-h" + c + "-c-k-no-nu-rw");
  }
  win.cardTypeOne = function (s, u) {
    ...
    if (f && f.length > 0) {
      ...
      for (var p = 0; p < f.length; ++p) {
        for (var m = f[p], g = 0; g < m.link.length; ++g) {
        ...
        var w = c(m.title.$t);
        var x = i(m.summary.$t);
        var h = x.substring(0, x.lastIndexOf(" ", 150));
        var v = m.author[0].name.$t;
        var y = !!m.author[0].uri && m.author[0].uri.$t;
        var k = m.published.$t;
        var I = m.updated.$t;
        var z = m.media$thumbnail ? a(m.media$thumbnail.url, "ratio16to9") : o;
        d +=
          ...
          ('<script type="application/ld+json">{"@context":"https://schema.org","@type":"BlogPosting","mainEntityOfPage":{"@type":"WebPage","@id":"' +
            ...
            '","url":"' +
            y +
            '"},"publisher":{"@type":"Organization","name":"Blogger","logo":{"@type":"ImageObject","url":"' +
            r +
            '"}},"datePublished":"' +
            k +
            '","dateModified":"' +
            I +
            '"}</script>') +
          "<div class='overflow-hidden h-100 border rounded-lg shadow-md bg-body bg-dm-dark-3'><a class='ratio ratio-w-16 ratio-h-9 d-block bg-dark bg-opacity-50 bg-dm-dark' aria-label='" +
          w +
          "' href='" +
          b +
          ...
          "' alt='" +
          w +
          "'/></a><div class='px-4 py-3'><h3 class='mb-1 fs-2 lh-22px text-hov-primary'><a class='clamp clamp-2' title='" +
          w +
          "' href='" +
          b +
          "'>" +
          w +
          "</a></h3><p class='clamp clamp-2 text-slate'>" +
          h +
          "</p></div></div></article>";
      }
      u.innerHTML = d;
  ...
  win.cardTypeTwo = function (s, u) {
    ...
    if (f && f.length > 0) {
      var d = "";
      for (var p = 0; p < f.length; ++p) {
        for (var g = f[p], b = 0; b < g.link.length; ++b) {
          if (g.link[b].rel == "alternate") {
            var w = g.link[b].href;
          }
        }
        var x = c(g.title.$t);
        var h = g.author[0].name.$t;
        var v = !!g.author[0].uri && g.author[0].uri.$t;
        var y = g.published.$t;
        var k = new Date(y).toLocaleDateString("id-ID", {
          weekday: "long",
          year: "numeric",
          month: "long",
          day: "numeric",
        });
        var I = new Date(y).toLocaleTimeString("id-ID", {
          hour: "2-digit",
          minute: "2-digit",
          hour12: false,
        });
        var z = g.updated.$t;
        var S = g.media$thumbnail ? a(g.media$thumbnail.url, "ratio16to9") : o;
        var L = g.content.$t;
        var Z = new DOMParser().parseFromString(L, "text/html");
        var _ = (t, n) => Z.querySelector(t)?.textContent || n;
        var M = _(".agenda-date span", k);
        var P = _(".agenda-time span", I + " s/d Selesai");
        var A = _(".agenda-location span", g.georss$featurename?.$t || "Indonesia");
        Z.querySelector(".agenda-description")?.remove();
        var m = i(Z.body.textContent);
        d +=
          "<article class='slider-inner'>" +
          ('<script type="application/ld+json">{"@context":"https://schema.org","@type":"BlogPosting","mainEntityOfPage":{"@type":"WebPage","@id":"' +
            w +
            ...
            x +
            '","description":"' +
            (m = m.substring(0, m.lastIndexOf(" ", 150))) +
            '","image":"' +
            S +
            '","author":{"@type":"Person","name":"' +
            h +
            ...
            z +
            '"}</script>') +
          "<div class='position-relative overflow-hidden mb-3 rounded-lg'><a class='ratio ratio-w-16 ratio-h-9 d-block bg-dark bg-opacity-50 bg-dm-dark' aria-label='" +
          x +
          "' href='" +
          w +
          "'><img class='lazy' data-src='" +
          ...
          "' alt='" +
          x +
          "'/></a><div class='position-absolute top-0 d-flex align-items-center gap-2 px-3 py-1 rounded-br-lg fw-medium fs-n1 bg-secondary text-dark'><svg class='svg-icon' aria-hidden='true'><use xlink:href='#date'/></svg><span>" +
          M +
          "</span></div></div><h3 class='mb-1 fs-2 lh-22px'><a class='clamp clamp-1' title='" +
          x +
          "' href='" +
          w +
          "'>" +
          x +
          " </a></h3><div class='d-flex flex-wrap gap-x-3'><div class='d-flex align-items-center gap-1 fs-n1 lh-22px'><svg class='svg-icon flex-shrink-0' aria-hidden='true'><use xlink:href='#map'/></svg><span class='clamp clamp-1'>" +
          A +
          "</span></div><div class='d-flex align-items-center gap-1 fs-n1 lh-22px'><svg class='svg-icon flex-shrink-0' aria-hidden='true'><use xlink:href='#time'/></svg><span class='clamp clamp-1'>" +
          P +
  ...
  win.cardTypeThree = function (s, u) {
    ...
    if (f && f.length > 0) {
      ...
      for (var p = 0; p < f.length; ++p) {
        ...
        var k = m.published.$t;
        var I = m.updated.$t;
        var z = m.media$thumbnail ? a(m.media$thumbnail.url, "ratio16to10") : o;
        d +=
          "<article class='slider-inner'>" +
          ('<script type="application/ld+json">{"@context":"https://schema.org","@type":"BlogPosting","mainEntityOfPage":{"@type":"WebPage","@id":"' +
            b +
            ...
            I +
            '"}</script>') +
          "<a class='group ratio ratio-w-16 ratio-h-10 overflow-hidden d-block rounded-lg bg-dark bg-opacity-90' href='" +
          b +
          "'><img class='group-hov-scale lazy' data-src='" +
          z +
          "' alt='" +
          w +
          "'/><div class='d-flex justify-content-center align-items-end p-3 text-center bg-gradient-2 bg-gradient-overlay'><h3 class='clamp clamp-2 fs-1 mb-0' title='" +
          w +
          "'>" +
          w +
          "</h3></div></a></article>";
  ...
  win.cardTypeFour = function (s, u) {
    ...
    if (f && f.length > 0) {
      var d = "";
      for (var p = 0; p < f.length; ++p) {
        for (var L = f[p], Z = 0; Z < L.link.length; ++Z) {
          if (L.link[Z].rel == "alternate") {
            var _ = L.link[Z].href;
          }
        }
        var v = c(L.title.$t);
        ...
        var I = (t, n) => k.querySelector(t)?.textContent || n;
        var z = I(".teacher-name", v);
        var S = I(".teacher-position", "Guru Profesional");
        k.querySelector(".table")?.remove();
        var h = i(k.body.textContent);
        h = h.substring(0, h.lastIndexOf(" ", 150));
        var m = L.author[0].name.$t;
        var g = !!L.author[0].uri && L.author[0].uri.$t;
        var b = L.published.$t;
        var w = L.updated.$t;
        var x = L.media$thumbnail ? a(L.media$thumbnail.url, "ratio3to4") : o;
        d +=
          "<article class='slider-inner'>" +
          ('<script type="application/ld+json">{"@context":"https://schema.org","@type":"BlogPosting","mainEntityOfPage":{"@type":"WebPage","@id":"' +
            _ +
            ...
            w +
            '"}</script>') +
          "<div class='position-relative overflow-hidden mb-3 rounded-lg'><a class='ratio ratio-w-3 ratio-h-4 d-block bg-dark bg-opacity-50 bg-dm-dark' aria-label='" +
          v +
          "' href='" +
          _ +
          "'><img class='lazy' data-src='" +
          ...
          "' alt='" +
          v +
          "'/></a><div class='position-absolute top-0 d-flex align-items-center gap-2 px-3 py-1 rounded-br-lg fw-medium fs-n1 bg-secondary text-dark'><svg class='svg-icon flex-shrink-0' aria-hidden='true'><use xlink:href='#user'/></svg><span class='clamp clamp-1'>" +
          S +
          "</span></div></div><h3 class='mb-0 fs-2 lh-22px'><a class='clamp clamp-1' title='" +
          v +
          "' href='" +
          _ +
          "'>" +
      ...
      for (var p = 0; p < f.length; ++p) {
        ...
        d +=
          ...
          ('<script type="application/ld+json">{"@context":"https://schema.org","@type":"BlogPosting","mainEntityOfPage":{"@type":"WebPage","@id":"' +
            ...
            I +
            '"}</script>') +
          "<a class='group ratio ratio-w-16 ratio-h-10 overflow-hidden d-block rounded-lg shadow-lg bg-dark bg-opacity-90' href='" +
          b +
          "'><img class='group-hov-scale lazy' data-src='" +
          z +
          "' alt='" +
  ...
  Array.from(doc.querySelectorAll(".fetch-load")).forEach(function (e) {
    ...
    fetch(blogUrl + "feeds/posts/" + i + "/-/" + u + "?max-results=" + a + "&alt=json")
      .then((t) => t.json())
      .then((t) => {
        const c = win[s];
      ...
      .then(() => lazyContent.update())
      .catch((t) => {
        e.innerHTML =
          "<p class='mx-auto my-5 px-3'>Anda belum melakukan setup wajib yang ada di panduan, silahkan hubungi admin.</p>";
      });
  });
}
runOptim(lazyOptim);
...
function fL() {
  ...
  doc.addEventListener("DOMContentLoaded", function () {
    if (e) {
      ...
      s.onload = function () {
        var c = JSON.parse(this.response).user;
        var s = new XMLHttpRequest();
        s.open("GET", "/feeds/posts/summary/?alt=json", true);
        s.onload = function () {
...
doc.querySelectorAll(".navbar-nav").forEach((t) => {
  ...
  o.forEach((t, a) => {
    ...
    const l = u.match(/onclick="([^"]*)"/);
    if (l) {
      t.setAttribute("onclick", l[1]);
      t.setAttribute("href", "#!");
    }
    const f = t.textContent.trim();
    const d = f.startsWith("_") && !f.startsWith("__");
    if (f.startsWith("__")) {
      ...
      if (a + 1 < o.length && o[a + 1].textContent.trim().startsWith("__")) {
        const { dropdown: t, menu: o } = i(u, n, true);
        c = o;
        if (r) {
          r.appendChild(t);
        }
      ...
      if (
        a + 1 < o.length &&
        o[a + 1].textContent.trim().startsWith("_") &&
        !o[a + 1].textContent.trim().startsWith("__")
      ) {
        const { dropdown: n, menu: o } = i(u, f);
        r = o;
    ...
    if (a === o.length - 1 && !f.startsWith("_")) {
      let n = "#mortarboard";
      const o = f.indexOf("@");
      let r = f;
      if (o !== -1) {
        r = f.substring(o + 1).trim();
        n = "#" + f.substring(0, o).trim();
      }
    ...
    e.forEach((t) => {
      if (t.isIntersecting) {
        ...
        const d = () => {
          if (l < s) {
            l += u;
            r.textContent = a ? f(Math.ceil(l)) + "+" : f(Math.ceil(l));
            setTimeout(d, 10);
          } else {
            r.textContent = a ? f(s) + "+" : f(s);
          }
        };
        d();
        o.unobserve(r);
...
(() => {
  ...
  function o() {
    const r =
      ((win.scrollY || html.scrollTop) / (html.scrollHeight - html.clientHeight)) * 100 * 3.6;
...
(() => {
  ...
  doc.querySelectorAll(".youtube").forEach((r) => {
    ...
    const l = "https://img.youtube.com/vi/" + i + "/" + s + ".jpg";
    const f = new Image();
    f.className = "yt-img lazy";
    f.dataset.src = l;
    f.src = "data:image/png;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=";
    f.alt = a;
    ...
    r.addEventListener("click", () => {
      e.classList.add("active");
      o.innerHTML =
        '<iframe class="border-0" title="' +
        a +
        '" allowfullscreen allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" src="https://www.youtube.com/embed/' +
        i +
        "?rel=0&showinfo=0&autoplay=1&loop=1&playlist=" +
        i +
        '"></iframe>';
...
(() => {
  ...
  if (o) {
    ...
    o.addEventListener("click", (a) => {
      ...
      f.onload = () => {
        if (f.status === 200) {
          const c = new DOMParser().parseFromString(f.response, "text/html");
...
(() => {
  if (body.classList.contains("postpage")) {
    ...
    if (o.length > 2) {
      ...
      c.innerHTML =
        '<button class="toc-btn toggle-collapse" type="button" aria-expanded="' +
        (i ? "false" : "true") +
        '"><svg class="svg-icon"><use xlink:href="#list-ul"/></svg>Daftar Isi</button><div class="collapse collapse-content' +
        (i ? "" : " show") +
        '"><div class="toc-inner"></div></div>';
      r.parentNode.insertBefore(c, r);
      c.setAttribute("data-track", "true");
      [].filter
        .call(doc.querySelectorAll(".toc-wrap"), (n) => !n.hasAttribute("data-track"))
        .forEach((n) => n.remove());
    }
    class r {
      constructor({ from: e, to: o }) {
        this.e = e;
        this.t = o;
...
doc.querySelectorAll(".accordion").forEach((t, n) => {
  ...
  t.querySelectorAll(".accordion-item").forEach((n) => {
    ...
    s.setAttribute("data-bs-target", "#" + l);
    s.setAttribute("aria-controls", l);
    c.add(l);
  });
});
(() => {
...
runOptim(() => {
  [...doc.querySelectorAll('[data-bs-toggle="tooltip"]')].map((n) => new bootstrap.Tooltip(n));
});
